...
export default class extends React.Component {
  ...
  componentDidMount() {
    ...
    window.test = d => {
      ...
      window.g = GSpread
      GSpread(
        { key: '19FQEKyzV7hHqDxJ3BSdNDNmxAa-otUXhmhHoJhv31wg' },
        (err, sheet) => {
          window.sheet = sheet
        },
      )
    ...
    this.setState({
      timezone:
        ...
        moment.tz(jstz.determine().name()).zoneAbbr(),
    })
    ...
    return props({
      ...
      cms:
        req.url.indexOf('test') > -1
          ? getRows('out9nrp', 'name')
          : getRows('orxrubb', 'name'),
    })
  }
  ...
  filterMatches(d) {
    ...
    if (this.state.filterPlayer) {
      results = results.filter(
        match =>
          match.team.indexOf(this.state.filterPlayer) > -1 ||
          match.team_2.indexOf(this.state.filterPlayer) > -1,
      )
    }
    ...
    if (this.state.filterTop) {
      // Helpers
      const getRating = (p, t) =>
        parseInt(_.get(d.players, [normalize(p), t], '1600'))
    ...
    results = results.filter(
      match => moment(new Date(match.time + ' UTC')) > expired,
    )
    ...
    return (
      <div className={general}>
        ...
        <div className={topNoteStyle}>
          ...
          <br />new to pro AoE2?{' '}
          <a
            href="https://medium.com/@charlieoffenbacher/5-minute-intro-to-pro-age-of-empires-2-94de4b3573b#.tc3kgqu8p"
            className={link}
          >
            learn more
          </a>
          <br />
          {this.state.timezone ? 'all times in ' + this.state.timezone : ''}
          <TimezoneSelector
            active={this.state.timezone}
            toggle={tz => this.setState({ timezone: tz })}
          />
        </div>
        {d.cms.header ? (
          <div dangerouslySetInnerHTML={{ __html: d.cms.header.content }} />
        ) : null}
        <hr />
        ...
        <div className={footerStyle}>
          ...
          <span>
            ...
            />
            &nbsp;<a href="https://github.com/coffenbacher">patao</a>, source
            available at{' '}
            <a href="https://github.com/coffenbacher/calendar">github</a>
          </span>
          <br />
          ...
          <span>
            <img
              className={miniFlag}
              src={_.get(d, ['flags', normalize('Netherlands'), 'url'])}
            />{' '}
            nimanoe
          </span>
          <br />
          <span>
            <img
              className={miniFlag}
              src={_.get(d, ['flags', normalize('Australia'), 'url'])}
            />{' '}
            robo
          </span>
          <br />
          ...
          <span>
            <img
              className={miniFlag}
              src={_.get(d, ['flags', normalize('Brazil'), 'url'])}
            />{' '}
            damianijr
          </span>
          <br />
          <span>
            <img
              className={miniFlag}
              src={_.get(d, ['flags', normalize('Japan'), 'url'])}
              style={{ boxShadow: '1px 1px 1px #888888' }}
            />{' '}
            iPhone
          </span>
          <br />
          <span>
            <img
              className={miniFlag}
              src={_.get(d, ['flags', normalize('Finland'), 'url'])}
              style={{ boxShadow: '1px 1px 1px #888888' }}
            />{' '}
            uhkis
          </span>
      ...
      const results = keyName
        ? // If there's a key
          rows.reduce((prev, cur) => {
            ...
            _.keys(_.pickBy(_.omit(cur, 'id'), _.isObjectLike)).map(k =>
              _.set(cur, k, ''),
            )
